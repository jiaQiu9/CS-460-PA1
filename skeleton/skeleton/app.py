...
def request_loader(request):
    ...
    cursor.execute(
        "SELECT password FROM Registered_Users WHERE email = '{0}'".format(email))
...
def getUsersPhotos(uid):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT photo_id, user_id, album_id, imgdata,  caption FROM Photos WHERE user_id = '{0}'".format(uid))
...
def getUserIdFromEmail(email):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT user_id  FROM Registered_Users WHERE email = '{0}'".format(email))
...
def protected():
    ...
    cursor.execute(
        "SELECT photo_id, user_id, album_id, imgdata, caption FROM Photos WHERE user_id=%s ", (uid))
    t = cursor.fetchall()
    for i in range(len(t)):
        ...
        #print("abum_id: ",t[i][2])
        cursor.execute(
            "SELECT album_name FROM Albums WHERE album_id=%s", t[i][2])
        album_name = cursor.fetchall()
        #print("album_name :")
        themes[i][5] = album_name[0][0]
    cursor.execute(
        "SELECT DISTINCT album_name FROM Albums WHERE user_id=%s", uid)
    all_album_name = cursor.fetchall()
    #print("album name of the current user",all_album_name[0][0])

    cursor.execute(
        "SELECT album_id, album_name FROM Albums WHERE user_id=%s", uid)
...
def personal_tags():
    ...
    uid = getUserIdFromEmail(flask_login.current_user.id)
    cursor.execute(
        "SELECT DISTINCT tag_name FROM Photo_has_tags as pht, Photos as p WHERE pht.photo_id=p.photo_id and user_id=%s", (uid))
...
def private_tagged_photos(variable):
    ...
    if len(t) != 0:
        ...
        cursor.execute(
            "SELECT album_name FROM Albums WHERE album_id=%s", t[i][3])
        album_name = cursor.fetchall()
...
def photos_in_album(variable):
    ...
    cursor.execute(
        "SELECT A.album_name FROM albums as A WHERE A.album_id=%s", variable)
...
def like_photo():
    if request.method == "POST":
        ...
        if len(check_likes) == 0:
            ...
            conn.commit()
            cursor.execute(
                'UPDATE registered_users SET contribution=contribution+1 WHERE user_id=%s', uid)
            conn.commit()
...
def upload_file():
    if request.method == 'POST':
        ...
        if (cursor.rowcount != 0):
            ...
            cursor.execute(
                "UPDATE Registered_Users AS R SET contribution = contribution + 1 WHERE R.user_id=%s", (uid))
            conn.commit()
            cursor.execute(
...
def home_page():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT photo_id, user_id, album_id, imgdata, caption FROM Photos")
    t = cursor.fetchall()
    for i in range(len(t)):
        #print("album name")
        themes.append({})
...
def list_friends():
    ...
    if friends_list != ():
        ...
        for i in friends_list:
            cursor.execute(
                "select email from registered_users where user_id = %s", i[0])
...
def show_top10():
    cursor.execute(
        "SELECT email from Registered_Users as R order by R.contribution desc limit 10")
